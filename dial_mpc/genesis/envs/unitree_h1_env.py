...
from dial_mpc.genesis.envs.base_env import BaseEnv, BaseEnvConfig
from dial_mpc.genesis.utils.function_utils import global_to_body_velocity, get_foot_step
from dial_mpc.utils.io_utils import get_model_path

@dataclass
class UnitreeH1WalkEnvConfig(BaseEnvConfig):
    kp: Union[float, torch.Tensor] = field(default_factory=lambda: torch.tensor(
        [
            200.0,
            200.0,
            200.0,  # left hips
    ...
    kd: Union[float, torch.Tensor] = field(default_factory=lambda: torch.tensor(
        [
            5.0,
            5.0,
            5.0,  # left hips
    ...
    def _make_scene(self, config: BaseEnvConfig, num_envs: int, render: bool) -> gs.Scene:
        self.scene = gs.Scene(
            ...
            sim_options=gs.options.SimOptions(
                dt=config.dt,
                substeps=int(config.dt / config.timestep),
            ),
            show_viewer=render,
        )
        self.plane = self.scene.add_entity(gs.morphs.Plane())
        self.robot: RigidEntity = self.scene.add_entity(gs.morphs.MJCF(
            file=str(get_model_path("unitree_h1", "mjx_scene_h1_walk.xml"))
        ))
        self.scene.build(n_envs=num_envs)
        return self.scene
    
    def _get_joint_ranges(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        physical_joint_range = self.robot.get_dofs_limit()
        physical_joint_range = torch.stack(physical_joint_range)
