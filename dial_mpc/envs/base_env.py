...
class BaseEnvConfig:
    ...
    kp: Union[float, List[float]] = 30.0
    ...
    dt: float = 0.02
    timestep: float = 0.02  # timestep of the underlying simulator step.
    backend: str = 'mjx'  # backend of the environment.


class BaseEnv(PipelineEnv):
    def __init__(self, config: BaseEnvConfig):
        assert config.timestep % config.dt == 0, 'timestep must be divisible by dt'
        self._config = config
